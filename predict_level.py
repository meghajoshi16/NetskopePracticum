...
model = torch.load("cnn_model_level.pth")
w2v_model = Word2Vec.load("word2vec.model")
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
label_level = {0: 'C-Level', 1: 'Contributor', 2: 'Director', 3: 'Executive', 4: 'Manager'}
...
def make_predictions():
    ...
    predict_label = [label_level[int(k)] for k in cnn_predictions]
    input_df["predicted_level"] = predict_label
